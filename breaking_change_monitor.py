    ...
    def compare_functionality(self) -> List[str]:
        ...
        for server_name in self.baseline_results.get("servers", {}):
            ...
            if current_tools < baseline_tools:
                issues.append(f"❌ {server_name}: Tool count decreased from {baseline_tools} to {current_tools}")
            ...
            for op_name, baseline_result in baseline_ops.items():
                ...
                if baseline_result.get("success") and not current_result.get("success"):
                    issues.append(f"❌ {server_name}: Operation '{op_name}' was working but now fails")
                ...
                if baseline_duration > 0 and current_duration > baseline_duration * 1.5:
                    issues.append(f"⚠️  {server_name}: Operation '{op_name}' is 50%+ slower ({current_duration:.3f}s vs {baseline_duration:.3f}s)")
...
async def main():
    ...
    parser.add_argument("--update-baseline", action="store_true", 
                       help="Update the baseline instead of comparing")
